...
def draw_main_cardinals(meter, contours, top_x, base_y, min_dist, min_range, max_range, skip_last_cardinal):
    i = 0
    ...
    ylist = []
    try:
        for cnt in contours[:skip_last_cardinal]:
            ...
            if min_range <= cv2.contourArea(cnt) <= max_range:
                ...
                if min_dist - 100 <= dist <= min_dist + 100: # this is final dist >= min_dist - 25:
        ...
        return
    return meter, contour_list, cardinal_cordinates, xlist, ylist
...
def draw_intermediate_cardinals(img_path, base_contour, top_x, top_y):
    meter = cv2.imread(img_path+"\\MeterImages\\cardinal.jpg")
    try:
